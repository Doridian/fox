...
build() {
  cd "${startdir}"
  go build -trimpath -ldflags "${goldflags} -X github.com/Doridian/fox/modules/info.version=${pkgver} -X github.com/Doridian/fox/modules/info.gitrev=$(git rev-parse HEAD)" -o "${srcdir}/fox" ./cmd
}
